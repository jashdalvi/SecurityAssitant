...
def create_mask(image_path):
    pic_path = image_path
    mask_path = "../data_aug/blue-mask.png"
    show = False
    model = "hog"
...
class FaceMasker:
    ...
    def _save(self):
        ...
        old_face_path = '-with-mask' + path_splits[1]
        path_list = self.face_path.split(os.path.sep)
        new_face_path = os.path.join(os.path.join("../Dataset_aug_advanced",path_list[-2]),path_list[-2] + path_list[-1][-6:-4])
        new_face_path = new_face_path + old_face_path
...
if __name__ == '__main__':
    ...
    for label in os.listdir(dirpath):
	    for i,imagep in enumerate(os.listdir(os.path.join(dirpath,label))):
		    if imagep.startswith("{}01".format(label)) or imagep.startswith("{}02".format(label)) or imagep.startswith("{}03".format(label)) or imagep.startswith("{}04".format(label)) or imagep.startswith("{}05".format(label)) or imagep.startswith("{}06".format(label)):
