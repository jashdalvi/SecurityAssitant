...
for i,imagepath in enumerate(imagepaths):
	...
	raw_bd = cv2.imread(beard_path,cv2.IMREAD_UNCHANGED)
	bd = raw_bd[:,:,:3]
	bd = bd[11:331,39:440]
	bdMask = raw_bd[:,:,3]
	bdMask = bdMask[11:331,39:440]   
	image = cv2.imread(imagepath)
	(H, W) = image.shape[:2]
	blob = cv2.dnn.blobFromImage(cv2.resize(image, (350, 350)), 1.0,
		(350, 350), (104.0, 177.0, 123.0))
	...
	(right_beard_index,left_beard_index)=FACIAL_LANDMARKS_IDXS['jaw']

	right_beard_pt = shape[right_beard_index]
	left_beard_pt = shape[left_beard_index-1]

	right_beard_pt = right_beard_pt.astype("int")
	left_beard_pt = left_beard_pt.astype("int")
	...
	#for beard
	dY_beard = right_beard_pt[1] - left_beard_pt[1]
	dX_beard = right_beard_pt[0] - left_beard_pt[0]
	print(dX_beard)
	angle_beard = np.degrees(np.arctan2(dY, dX)) - 180
	...
	#bd = imutils.rotate_bound(bd,angle_beard)
	...
	#print(sg.shape)
	# for beard
	#bdW = int((shape[16][0] - shape[0][0])/np.cos(np.radians(-angle_beard)))
	bd = imutils.rotate_bound(bd,angle_beard)
	bdW = int((shape[16][0] - shape[0][0]))
	bdH = int((shape[8][1] - shape[27][1])*1.25)
	bd = cv2.resize(bd, (bdW,bdH),cv2.INTER_NEAREST)
	#bd = imutils.rotate_bound(bd,angle_beard)
	...
	#for beard
	bdMask = cv2.threshold(bdMask, 0, 255, cv2.THRESH_BINARY)[1]
	bdMask = imutils.rotate_bound(bdMask, angle_beard)
	#bdMask = imutils.resize(bdMask, width=bdW, inter=cv2.INTER_NEAREST)
	bdMask = cv2.resize(bdMask,(bdW,bdH),cv2.INTER_NEAREST)
	#bdMask = imutils.rotate_bound(bdMask, angle_beard)

	#shiftX = int(sg.shape[1] * 0.25)
	#shiftY = int(sg.shape[0] * 0.35)

	shiftX_beard = int(bd.shape[1] * 0.2)
	shiftY_beard = int(bd.shape[0] * 0.35)

	#output = overlay_image(image, sg, sgMask,
	#		(rightEyeCenter[0] - shiftX, rightEyeCenter[1] - shiftY))

	print((right_beard_pt[0],right_beard_pt[1]))
	output_beard = overlay_image(image,bd,bdMask,(right_beard_pt[0],right_beard_pt[1]))
	...
	if not os.path.exists(label_path):
		os.mkdir(label_path)
	cv2.imwrite(os.path.join(label_path,"{}beard{}.jpg".format(label,str(i+1))),output_beard)
